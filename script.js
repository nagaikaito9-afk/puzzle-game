...
function login() {
    ...
    database.ref('users/' + id).once('value', snapshot => {
        if (snapshot.exists() && snapshot.val().password === pass) { 
            ...
            showCustomDialog('alert', "IDかパスワードが違います");
        }
    });
}
...
function register() {
    ...
    if (accCount >= 5) return showCustomDialog('alert', "上限(5個)に達しました");
    
    const id = document.getElementById('reg-id').value;
    const pass = document.getElementById('reg-pass').value;
    if (!id || !pass) return showCustomDialog('alert', "入力してください"); 
    if (pass.length < 6) return showCustomDialog('alert', "6文字以上必要です");
...
async function deleteAccount() { 
    if (await showCustomDialog('confirm', "本当にアカウントを削除しますか？")) { 
...
async function sendFriendRequest() { 
    const targetId = await showCustomDialog('prompt', "リクエストを送るアカウント名"); 
    ...
    database.ref('users/' + targetId).once('value', snapshot => { 
        if (snapshot.exists()) { 
            ...
            showCustomDialog('alert', "リクエストを送信しました！"); 
        } else {
            showCustomDialog('alert', "見つかりませんでした"); 
        }
    }); 
}
...
async function removeFriend(friendId) { 
    if (await showCustomDialog('confirm', "フレンドから削除しますか？")) { 
...
function showFriendsScreen() {
    ...
    const reqC = document.getElementById('friend-requests-container');
    const frC = document.getElementById('friends-list-container');
    
    database.ref(`users/${currentUser}/friendRequests`).on('value', snap => { 
        reqC.innerHTML = ''; 
        if (!snap.exists()) return reqC.innerHTML = '<p>なし</p>'; 
        snap.forEach(child => { 
            const div = document.createElement('div'); 
            div.innerHTML = `<span>👤 ${child.key}</span> <div><button onclick="acceptRequest('${child.key}')">承認</button><button onclick="rejectRequest('${child.key}')" style="background:#d9534f;">拒否</button></div>`; 
            reqC.appendChild(div); 
        }); 
    });
    
    database.ref(`users/${currentUser}/friends`).on('value', snap => { 
        frC.innerHTML = ''; 
        if (!snap.exists()) return frC.innerHTML = '<p>フレンドがいません</p>'; 
        snap.forEach(child => { 
            const div = document.createElement('div'); 
            div.innerHTML = `<button onclick="showCourseList('friend', null, '${child.key}')">👤 ${child.key} のコース</button><button onclick="removeFriend('${child.key}')" style="background:#d9534f;">削除</button>`; 
            frC.appendChild(div); 
        }); 
    });
}
...
function showCourseList(type, sortOrder, friendId = null) {
    ...
    const t = document.getElementById('course-list-title');
    const s = document.getElementById('course-search');
    c.innerHTML = '読込中...'; 
    s.style.display = type === 'world' ? 'block' : 'none'; 
    s.value = '';
    
    const render = (courses) => { 
        currentLoadedCourses = courses.map((co, i) => ({ ...co, originalIndex: i })); 
        filterCourses(); 
    };
    ...
    if (type === 'world') { 
        ...
        database.ref('worldCourses').once('value', snap => { 
            let cs = []; 
            snap.forEach(ch => { cs.push({ key: ch.key, ...ch.val() }); }); 
            if (sortOrder === 'popular') cs.sort((a, b) => (b.likes || 0) - (a.likes || 0)); 
            else cs.reverse(); 
            render(cs); 
        }); 
    } 
...
function filterCourses() {
    ...
    f.forEach((co) => { 
        ...
        b.innerText = `[${co.gameMode||'3D'}] ${co.name} (❤️${co.likes || 0}) / 作者: ${co.author || '自分'}`; 
        b.onclick = () => { viewingCourseIndex = co.originalIndex; showCourseDetail(co); }; 
        c.appendChild(b); 
    });
}
...
function showCourseDetail(course) {
    ...
    gameCourseMode = course.gameMode || '3D';
    
    document.getElementById('detail-title').innerText = course.name; 
    document.getElementById('detail-author').innerText = course.author || "自分"; 
    ...
    const l = document.getElementById('like-btn'); 
    l.innerText = "❤️ いいね！"; 
    l.disabled = false;
    
    if (viewingCourseKey) { 
        database.ref(`worldCourses/${viewingCourseKey}/likedUsers/${currentUser}`).once('value', s => { 
            if (s.exists()) l.innerText = "💔 いいね解除"; 
        }); 
    } else {
    ...
    cS.innerHTML = ''; 
    if (course.comments) {
        Object.values(course.comments).forEach(c => cS.innerHTML += `<div class="comment"><b>${c.user}</b>: ${c.text}</div>`);
    }
    showScreen('course-detail-screen');
}

async function deleteCourse() { 
    if (await showCustomDialog('confirm', "本当にこのコースを削除しますか？")) { 
...
function likeCourse() {
    ...
    const r = database.ref(`worldCourses/${viewingCourseKey}`); 
    const l = document.getElementById('like-btn'); 
    l.disabled = true;
    
    r.child(`likedUsers/${currentUser}`).once('value', snap => {
        if (!snap.exists()) { 
            r.child(`likedUsers/${currentUser}`).set(true); 
            r.child('likes').transaction(likes => (likes || 0) + 1); 
            l.innerText = "💔 いいね解除"; 
            document.getElementById('detail-likes').innerText = (parseInt(document.getElementById('detail-likes').innerText) || 0) + 1; 
        } else { 
            r.child(`likedUsers/${currentUser}`).remove(); 
            r.child('likes').transaction(likes => Math.max((likes || 0) - 1, 0)); 
            l.innerText = "❤️ いいね！"; 
            document.getElementById('detail-likes').innerText = Math.max((parseInt(document.getElementById('detail-likes').innerText) || 0) - 1, 0); 
        }
        l.disabled = false;
    });
}
...
function postComment() { 
    if (!viewingCourseKey) return; 
    const t = document.getElementById('comment-select').value; 
    database.ref(`worldCourses/${viewingCourseKey}/comments`).push({ user: currentUser, text: t }); 
    document.getElementById('comment-section').innerHTML += `<div class="comment"><b>${currentUser}</b>: ${t}</div>`; 
    document.getElementById('post-comment-btn').innerText = "送信完了！"; 
    setTimeout(() => { document.getElementById('post-comment-btn').innerText = "コメント送信"; }, 1500); 
}
...
const head = new THREE.Mesh(new THREE.SphereGeometry(0.4, 32, 32), bodyMat); 
player.add(head);
...
const eyeL = new THREE.Mesh(new THREE.SphereGeometry(0.05, 16, 16), eyeMat); 
eyeL.position.set(-0.15, 0.1, 0.35); 
player.add(eyeL);

const eyeR = new THREE.Mesh(new THREE.SphereGeometry(0.05, 16, 16), eyeMat); 
eyeR.position.set(0.15, 0.1, 0.35); 
player.add(eyeR);

const nose = new THREE.Mesh(new THREE.SphereGeometry(0.06, 16, 16), new THREE.MeshBasicMaterial({ color: 0xff6666 })); 
...
let linkLinesGroup = new THREE.Group(); 
...
function getCanvasTexture(type) {
    ...
    if (type === 'fake') { 
        ctx.fillStyle='#333'; ctx.fillRect(0,0,64,64); ctx.strokeStyle='#ff66b2'; ctx.lineWidth=6; ctx.beginPath(); ctx.moveTo(10,10); ctx.lineTo(54,54); ctx.moveTo(54,10); ctx.lineTo(10,54); ctx.stroke(); 
    } else if (type === 'door') { 
        ctx.fillStyle='#d2b48c'; ctx.fillRect(0,0,64,64); ctx.fillStyle='#553311'; ctx.beginPath(); ctx.arc(32, 24, 8, 0, Math.PI*2); ctx.fill(); ctx.beginPath(); ctx.moveTo(26,44); ctx.lineTo(38,44); ctx.lineTo(34,24); ctx.lineTo(30,24); ctx.fill(); 
    } else if (type === 'warp' || type === 'locked_warp') { 
        ctx.fillStyle = type==='warp' ? '#ffb6c1' : '#ff4500'; ctx.fillRect(0,0,64,64); ctx.strokeStyle='#fff'; ctx.lineWidth=4; ctx.beginPath(); for(let i=1;i<20;i++){ ctx.arc(32,32, i*1.5, 0, Math.PI*2); } ctx.stroke(); 
    } else if (type === 'ice') { 
        ctx.fillStyle='#e0ffff'; ctx.fillRect(0,0,64,64); ctx.strokeStyle='#fff'; ctx.lineWidth=2; ctx.beginPath(); ctx.moveTo(32,10); ctx.lineTo(32,54); ctx.moveTo(10,32); ctx.lineTo(54,32); ctx.moveTo(16,16); ctx.lineTo(48,48); ctx.moveTo(48,16); ctx.lineTo(16,48); ctx.stroke(); 
    } else if (type === 'conveyor') { 
        ctx.fillStyle='#ffb347'; ctx.fillRect(0,0,64,64); ctx.fillStyle='#fff'; ctx.font="40px Arial"; ctx.textAlign="center"; ctx.textBaseline="middle"; ctx.fillText('↑', 32, 36); 
    } else if (type === 'sand') { 
        ctx.fillStyle='#f5deb3'; ctx.fillRect(0,0,64,64); ctx.fillStyle='#cd853f'; for(let i=0;i<50;i++){ ctx.fillRect(Math.random()*64, Math.random()*64, 2, 2); } 
    } else if (type === 'metal') { 
        ctx.fillStyle='#dcdcdc'; ctx.fillRect(0,0,64,64); ctx.strokeStyle='#fff'; ctx.lineWidth=4; ctx.strokeRect(4,4,56,56); ctx.fillStyle='#a9a9a9'; ctx.beginPath(); ctx.arc(8,8,3,0,7); ctx.arc(56,8,3,0,7); ctx.arc(8,56,3,0,7); ctx.arc(56,56,3,0,7); ctx.fill(); 
    } else { 
...
const extraColors = [
    ...
    {id:'p_wood', c:0xf5deb3}, {id:'p_stone', c:0x778899}, {id:'p_gold', c:0xffd700}, {id:'p_dirt', c:0xd2b48c},
    {id:'p_leaf', c:0x3cb371}, {id:'p_cloud', c:0xf8f8ff}, {id:'p_neon', c:0x39ff14}, {id:'p_choco', c:0xd2691e}, 
    {id:'p_mint', c:0xf5fffa}, {id:'p_cherry', c:0xff1493}, {id:'p_plum', c:0xdda0dd}, {id:'p_navy', c:0x000080}, 
    {id:'p_olive', c:0x6b8e23}, {id:'p_teal', c:0x008080}, {id:'p_silver', c:0xc0c0c0}, {id:'p_bronze', c:0xcd7f32}
];
extraColors.forEach(e => BLOCKS.push({id: e.id, n: '装飾 '+e.id, c: e.c}));

let solidBlocks=[], customDeathBlocks=[], customWarps=[], customGoal=null, customStart=null;
let placed=new Set(), meshList=[], customKeys=[], customDoors=[], hasKeys=[]; 

// --- ブロック管理・エディタ関数 ---
function clearScene() { 
    meshList.forEach(m => scene.remove(m)); 
    solidBlocks=[]; customDeathBlocks=[]; customWarps=[]; customGoal=null; customStart=null; 
    customKeys=[]; customDoors=[]; placed.clear(); meshList=[]; hasKeys=[]; linkLinesGroup.clear();
}
...
function initSidebar() {
    ...
    if (gameCourseMode === '2D') {
        BLOCKS.forEach(base => {
            if (!base.pass && base.id !== 'start' && base.id !== 'goal' && base.id !== 'key' && base.id !== 'door' && !base.id.includes('warp')) {
                activeBlocks.push({...base, id: base.id + '_half', n: base.n + '(ハーフ)', half: true});
            }
        });
    }
    ...
    activeBlocks.forEach(b => {
        ...
        btn.innerHTML = `<span class="block-title">${b.n}</span>`;
        c.appendChild(btn);
    });
}
...
function createMesh(bDef) {
    if (bDef.type === 'goal') { 
        ...
        const p = new THREE.Mesh(new THREE.CylinderGeometry(0.05,0.05,2), new THREE.MeshLambertMaterial({color:0xffaa44})); 
        p.position.y=1; g.add(p); 
        const f = new THREE.Mesh(new THREE.PlaneGeometry(0.8,0.6), new THREE.MeshLambertMaterial({color:0xff66b2, side:THREE.DoubleSide})); 
        f.position.set(0.4, 1.7, 0); g.add(f); 
        return g; 
    }
    if (bDef.type === 'start') { 
        const g = new THREE.Group(); 
        const p = new THREE.Mesh(new THREE.CylinderGeometry(0.6,0.6,0.2), new THREE.MeshLambertMaterial({color:0xffaa44})); 
        p.position.y=0.1; g.add(p); 
        return g; 
    }
    if (bDef.type === 'key') { 
        const g = new THREE.Group(); 
        const b = new THREE.Mesh(new THREE.ConeGeometry(0.15,0.4,16), new THREE.MeshLambertMaterial({color:0x87cefa})); 
        b.rotation.z = -Math.PI/2; g.add(b); 
        const t = new THREE.Mesh(new THREE.ConeGeometry(0.1,0.2,16), new THREE.MeshLambertMaterial({color:0x87cefa})); 
        t.rotation.z = Math.PI/2; t.position.x = -0.25; g.add(t); 
        g.position.y = 0.5; 
        return g; 
    }
    if (bDef.half) { 
        const m = new THREE.Mesh(new THREE.BoxGeometry(1,0.5,1), getBlockMaterial(bDef)); 
        m.position.y -= 0.25; 
        return m; 
    }
...
function removeBlockMesh(m) {
    ...
    customWarps = customWarps.filter(b => b !== m); 
    customKeys = customKeys.filter(b => b !== m); 
    customDoors = customDoors.filter(b => b !== m);
...
function placeBlock(type, pos, save=true, playSound=true, loadUuid=null, loadWarp=null, loadLock=null, loadDir=0) {
    ...
    if (type === 'eraser') { 
        if (placed.has(posKey)) { 
            ...
            if (m) { removeBlockMesh(m); playSE('place'); } 
        } 
        return; 
    }
    ...
    if (['goal','start'].includes(type)) { 
        let ex = type==='goal' ? customGoal : customStart; 
        if(ex) removeBlockMesh(ex); 
    }
    ...
    const mesh = createMesh(fullDef); 
    mesh.position.copy(pos); 
    
    if (baseId === 'conveyor') { 
        mesh.rotation.y = -loadDir * Math.PI/2; 
    }
    ...
    if (baseId==='goal') customGoal=mesh; 
    else if (baseId==='start') customStart=mesh;
    else if (baseId==='key') customKeys.push(mesh); 
    else if (baseId==='door') customDoors.push(mesh);
...
function editCourseFromDetail() { 
    ...
    viewingCourseData.forEach(b => { 
        placeBlock(b.type, new THREE.Vector3(b.x, b.y, b.z), true, false, b.uuid, b.warpTargetId, b.lockId, b.dir); 
    }); 
    resetPlayerPosition(); 
    drawLinkLines(); 
}
...
function startEditor(mode) { 
    ...
    currentCourseData = []; 
    document.getElementById('editor-mode-display').innerText = `モード: エディタ (${gameCourseMode})`;
    camPanX = 0; camPanZ = 0; camPanY = 0; camTheta = 0; 
    camPhi = gameCourseMode === '2D' ? Math.PI/2 : 1.0; 
    camRadius = 15;
    initSidebar(); 
    selectBlock('normal'); 
...
async function saveLocalCourse() { 
    const name = await showCustomDialog('prompt', "コース名を入力", "マイコース"); 
    if (name) { 
        myCourses.push({ name: name, gameMode: gameCourseMode, data: JSON.parse(JSON.stringify(currentCourseData)) }); 
        localStorage.setItem('myCourses', JSON.stringify(myCourses)); 
        showCustomDialog('alert', "保存しました！"); 
        quitPlay(); 
    } 
}

async function publishCourse() { 
    if (!currentUser) return showCustomDialog('alert', "ログインが必要です"); 
    const name = await showCustomDialog('prompt', "公開するコース名", "マイコース"); 
    if (name) { 
        const d = JSON.parse(JSON.stringify(currentCourseData)); 
        myCourses.push({ name: name, gameMode: gameCourseMode, data: d }); 
        localStorage.setItem('myCourses', JSON.stringify(myCourses)); 
        
        database.ref('worldCourses').push({ 
            name: name, author: currentUser, gameMode: gameCourseMode, data: d, likes: 0, plays: 0, clears: 0 
        }).then(() => { 
            showCustomDialog('alert', "世界に公開しました！"); 
            quitPlay(); 
        }).catch(err => { 
            showCustomDialog('alert', "エラー: " + err.message); 
            quitPlay(); 
        }); 
    } 
}
...
async function handleRightClickLink(cx, cy) {
    ...
    if (intersects.length > 0) {
        ...
        while(m.parent && m.parent.type==='Group') m = m.parent;
        ...
        if (['key','door','warp','locked_warp'].includes(t)) {
            if (!linkSourceMesh) {
                if (m.userData.warpTargetId || m.userData.lockId) {
                    if (await showCustomDialog('confirm', "🔗 ペアを解除しますか？")) {
                        if (m.userData.warpTargetId) { 
                            let tgt = meshList.find(b => b.userData.uuid === m.userData.warpTargetId); 
                            if (tgt) tgt.userData.warpTargetId = null; 
                            m.userData.warpTargetId = null; 
                        }
                        if (m.userData.lockId) { 
                            let tgt = meshList.find(b => b.userData.uuid === m.userData.lockId); 
                            if (tgt) tgt.userData.lockId = null; 
                            m.userData.lockId = null; 
                        }
                        updateCourseDataLinks(); drawLinkLines(); 
                        return showCustomDialog('alert', "💔 ペアを解除しました！");
                    }
                }
                linkSourceMesh = m; 
                showCustomDialog('alert', "【ペア元を選択しました】\nもう一度、対応させたいブロックを右クリックしてください。");
            } else {
                if (linkSourceMesh === m) { 
                    linkSourceMesh=null; 
                    return showCustomDialog('alert', "キャンセルしました"); 
                }
                
                const t1 = linkSourceMesh.userData.type.replace('_half',''); 
                const t2 = m.userData.type.replace('_half',''); 
                let success = false;
                ...
                if (success) { 
                    ...
                    showCustomDialog('alert', "🔗 ペアを構築しました！"); 
                    linkSourceMesh = null; 
                    drawLinkLines(); 
                } else { 
                    showCustomDialog('alert', "⚠️ その組み合わせではペアを組めません"); 
                    linkSourceMesh = null; 
                }
            }
        }
...
function updateCourseDataLinks() { 
    meshList.forEach(m => { 
        let d = currentCourseData.find(cd => cd.uuid === m.userData.uuid); 
        if (d) { 
            d.warpTargetId = m.userData.warpTargetId; 
            d.lockId = m.userData.lockId; 
        } 
    }); 
}
...
function drawLinkLines() {
    ...
    meshList.forEach(m1 => {
        if (m1.userData.warpTargetId && !dW.has(m1.userData.uuid)) {
            ...
            if (m2) { 
                linkLinesGroup.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([m1.position, m2.position]), new THREE.LineBasicMaterial({color: 0xff0000, linewidth: 3}))); 
                dW.add(m1.userData.uuid); dW.add(m2.userData.uuid); 
            }
        }
        if (m1.userData.lockId && !dL.has(m1.userData.uuid)) {
            const m2 = meshList.find(b => b.userData.uuid === m1.userData.lockId);
            if (m2) { 
                linkLinesGroup.add(new THREE.Line(new THREE.BufferGeometry().setFromPoints([m1.position, m2.position]), new THREE.LineBasicMaterial({color: 0xffaa00, linewidth: 3}))); 
                dL.add(m1.userData.uuid); dL.add(m2.userData.uuid); 
            }
        }
    });
}
...
window.addEventListener('mousemove', e => {
    if (isDragging) {
        if (gameCourseMode === '2D') { 
            camPanX -= (e.clientX - prevX) * 0.03; 
            camPanY += (e.clientY - prevY) * 0.03; 
        } else {
            if (e.shiftKey && currentMode === 'editor') { 
                let dx = (e.clientX - prevX) * 0.05; 
                let dy = (e.clientY - prevY) * 0.05; 
...
function resetPlayerPosition() {
    ...
    warpCooldown=0; isCrouching=false; player.scale.y = 1.0;
...
function updatePhysics() {
    ...
    if (onBlock) {
        ...
        if(t==='speed_1') moveSpd=0.2; 
        if(t==='speed_2') moveSpd=0.35; 
        if(t==='slow'||t==='sand') moveSpd=0.03;
        
        if(t==='conveyor') {
            let dir = onBlock.userData.dir || 0;
            if(dir===0) player.position.z-=0.05; 
            if(dir===2) player.position.z+=0.05; 
            if(dir===1) player.position.x+=0.05; 
            if(dir===3) player.position.x-=0.05;
    ...
    if((keys.space || (gameCourseMode==='2D' && (keys.w || keys.up))) && isGrounded && !isCrouching) { 
        velocityY=0.22; keys.space=false; keys.w=false; keys.up=false; playSE('jump'); 
    }
...
function updateCam() {
    ...
    if (gameCourseMode === '2D') {
        player.visible = true; 
        camera.position.set(targetX, targetY + 2, 15); 
        camera.lookAt(targetX, targetY + 2, 0);
    } else {
        if (isFirstPerson && (currentMode === 'game' || currentMode === 'test')) {
            player.visible = false; 
            camera.position.set(player.position.x, player.position.y + (player.scale.y===0.5 ? -0.1 : 0.4), player.position.z);
            ...
            player.visible = true; 
            camera.position.set(targetX + camRadius * Math.sin(camPhi) * Math.sin(camTheta), targetY + camRadius * Math.cos(camPhi), targetZ + camRadius * Math.sin(camPhi) * Math.cos(camTheta));
...
function animate() {
    ...
    if ((currentMode==='game' || currentMode==='test') && !isCleared) {
        let ix=0, iz=0; 
        if (gameCourseMode === '2D') { 
            if(keys.left||keys.a) ix-=1; if(keys.right||keys.d) ix+=1; player.position.z = 0; 
        } else { 
        ...
        if(ix!==0 || iz!==0) {
            ...
            if (gameCourseMode === '2D') { 
                dx = ix * speed; dz = 0; player.rotation.y = ix > 0 ? Math.PI/2 : -Math.PI/2; 
            } else { 
